...
def setup_database():
    ...
    try:
        ...
        if not env_loaded:
            logger.error("❌ No .env.pythonanywhere file found in any expected location")
...
def get_vector_store():
    """Get vector store instance (lazily initialized)"""
    global _vector_store
    if _vector_store is None:
        try:
            logger.info("Initializing PythonAnywhere Vector Store (Qdrant)")
            if 'VectorStore' not in globals():
                # Import VectorStore if not already imported
                try:
                    from .vector_store_pa import VectorStore
                except ImportError:
                    from deployment.pythonanywhere.database.vector_store_pa import VectorStore
            ...
            logger.info("✅ PythonAnywhere Vector Store initialized successfully")
        except Exception as e:
            logger.error(f"❌ Error initializing PA Vector Store: {e}")
            raise
